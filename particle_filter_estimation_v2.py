...
class particle_filter:
    ...
    def initialize_particles(self):
        ...
        for _ in range(self.num_particles):
            particle_pos = random.choice(self.env.initial_states)
    ...
    def calculate_entropy_seq(self, obs_seq, act_seq):
        num_steps = len(obs_seq)
        for i in range(num_steps):
            self.predict(act_seq[i])
            self.update(obs_seq[i], act_seq[i])
            self.resample()
        ...
        for st_T in self.env.secret_states:
            p_zT1 += probs[st_T]
        p_zT0 = 1 - p_zT1
        # print("the probability of being in the secret: ", p_zT1)
